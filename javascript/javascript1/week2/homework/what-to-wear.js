...
function whatToWear(temp) { //passing a temp parameter for evaluation inside the function
    ...
    if (temp > 18 || temp == 18) { //if statement temp will be equal or higher than 18 we will use summer clothes. 
        ...
        console.log(`The temperature today is likely to stay above ${temp} so you can go out in shorts and a t-shirt.`); //using template string / template literals - dymamic variables inside a string. This way we don't need to concatenate the temp with the string. 
...
let clothesToWear = whatToWear(18);
