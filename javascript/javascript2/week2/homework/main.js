...
const result = generateListings(37);
result.forEach((listing) => console.log(listing.size))
// console.log(result);

// const listPrice = generateListings(37);
const arr = result.map((prices) => prices.price)
console.log(arr);


const cheapListings = generateListings(37);
const obj = cheapListings.filter((cheap) => cheap.price <= 5000)
console.log(obj);

const expList = result.filter((value) => value.price >= 5000)
.map((value) => value.price)
console.log(expList);

const hasPark = result.filter((park) => park.facilities.includes('Parkering'))
console.log(hasPark);
