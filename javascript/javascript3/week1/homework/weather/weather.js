  ...
  window.addEventListener('load', ()=> {
    ...
    if(navigator.geolocation){
      navigator.geolocation.getCurrentPosition(position => {
        ...
        const api = `http://cors.io/?https://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${long}&id=daf90bf3cb90d58f344174ca345725da`;
        fetch(api)
          .then(r => {
            console.log(r);
            return r.json();
...
document.getElementById ('cityWeather').onclick = function () {
  ...
  fetch (
    `http://cors.io/?https://api.openweathermap.org/data/2.5/weather?q=${input.value}&appid=daf90bf3cb90d58f344174ca345725da&containerid:'openweathermap-widget-12'`
  )
    .then (res => res.json ())
    .then (data => {
      console.log (data);
